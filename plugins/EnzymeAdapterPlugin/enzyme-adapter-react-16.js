...
function toTree(vnode) {
    ...
    const node = (0, _reflection.findCurrentFiberUsingSlowPath)(vnode);
    switch (node.tag) {
        case HostRoot:
            // 3
...
class ReactSixteenAdapter extends _enzyme.EnzymeAdapter {
    constructor() {
        super();
        ...
        return {
            render(el, context, callback) {
                if (instance === null) {
                    ...
                    const wrapperProps = _extends({
                        Component: type,
                        props,
                        context
                    }, ref && {ref});
                    const ReactWrapperComponent = (0, _enzymeAdapterUtils.createMountWrapper)(el, options);
                    const wrappedEl = _react2.default.createElement(ReactWrapperComponent, wrapperProps);
            ...
            simulateEvent(node, event, mock) {
                const mappedEvent = (0, _enzymeAdapterUtils.mapNativeEventNames)(event);
                const eventFn = _testUtils2.default.Simulate[mappedEvent];
                if (!eventFn) {
    ...
    createShallowRenderer() /* options */ {
        const renderer = new _shallow2.default();
        let isDOM = false;
        let cachedNode = null;
        return {
            render(el, context) {
                ...
                if (typeof el.type === 'string') {
                    ...
                    isDOM = false;
                    return (0, _enzymeAdapterUtils.withSetStateAllowed)(() => renderer.render(el, context));
                }
            },
            unmount() {
                renderer.unmount();
            ...
            simulateEvent(node, event, ...args) {
                const handler = node.props[(0, _enzymeAdapterUtils.propFromEvent)(event)];
                if (handler) {
                    (0, _enzymeAdapterUtils.withSetStateAllowed)(() => {
        ...
        return {
            render(el, context) {
                if (options.context && (el.type.contextTypes || options.childContextTypes)) {
                    const childContextTypes = _extends({}, el.type.contextTypes || {}, options.childContextTypes);
                    const ContextWrapper = (0, _enzymeAdapterUtils.createRenderWrapper)(el, context, childContextTypes);
    ...
    createElement(...args) {
        return _react2.default.createElement(...args);
    }
}
