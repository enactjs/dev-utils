			...
			if(file === opts.chunk) {
				...
				for(let i=0; i<locales.length; i++) {
					try {
						...
						compilation.applyPlugins('prerender-locale', {chunk:opts.chunk, opts:renderOpts});
...
function simplifyAliases(locales, status) {
	...
	const remove = (targets, classes) => classes.split(/\s+/).filter(c => !targets.includes(c)).join(' ');
	let multiCount = 1;
	...
	for(let i=0; i<status.alias.length; i++) {
		if(status.alias[i]) {
			...
			if(!sharedCSS[status.alias[i]]) {
				...
				sharedCSS[status.alias[i]] = common(sharedCSS[status.alias[i]],
